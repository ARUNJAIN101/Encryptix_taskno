...
class TodoListApp(tk.Tk):
    def __init__(self):
        ...
        self.configure("Custom.TEntry", foregroud="gray")
...
# set placeholder for input fied
self.task_input.insert(0, "Enter your task here...")

# Bind event to clear placeholder when input field loses focus
self.task_input.bind("<FocusIn>", self.clear_placeholder)
# Bind event to restore placeholder when input field
self.task_input.bind("<FocusIn>", self.restore_placeholer)
...
# create buttons for marking as done or deleting them
ttk.Button(self, text="Mark as done", style="success.TButton",
         command=self.mark_as_done).pack(side=tk.LEFT, padx=10, pady=10)
...
ttk.Button(self, text="View Status", style="info.TButton",
           command=self.view_status).pack(side=tk.BOTTOM, pady=10)
